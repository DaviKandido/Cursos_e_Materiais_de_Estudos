...
// Create a route (Rota)
app.get("/api", (rec: Request,res: Response )=>{ // REC = Requisição, RES = Resposta
...
function buscaPostPorId(id: number) { 
...
app.get("/posts/:id", (rec: Request,res: Response )=>{ 

    const index = buscaPostPorId(Number(rec.params.id)); // Get the id from the request
